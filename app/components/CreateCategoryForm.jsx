    ...
    <div className="max-w-md mx-auto bgColor fontColor rounded-lg shadow-lg p-6 mb-5 border-2 borderColor">
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <label
            htmlFor="name"
            className="block text-sm font-medium fontColorGray "
          >
            Name:
          </label>
          <input
        ...
        <div className="flex flex-col gap-1 mb-4 ">
          <div className="flex justify-between min-[200px]:items-end w-full max-[200px]:flex-col max-[200px]:justify-start ">
            Products
            <button
              className={`rounded-full px-2 py-1 text-sm bg-gray-200 hover:bg-gray-300 opBgColor opTxtColor ${
                includedProducts.length === 0
                  ? "cursor-not-allowed opacity-50"
                  : "cursor-pointer"
              }`}
              onClick={() => setIncludedProducts([])}
              disabled={includedProducts.length === 0}
            >
              Deselect All
            </button>
            {/* <label htmlFor="deSelectAll" className={`flex gap-2 cursor-pointer ${includedProducts.length === 0 ? 'opacity-50' : ''}`}>
              Deselect All
              <input
          ...
          <div className="border-2 borderColor rounded-lg p-2 pt-4 ">
            ...
            {includedProducts.length > 0 && (
              <div className="bgColorGray w-3/4 h-1 rounded-full mt-2 mb-4 "></div>
            )}
            <div className="flex flex-wrap gap-2 ">
              {products.map((product) => {
                let isIncluded = false;
                includedProducts.map((includedProduct) => {
                  if (includedProduct.id === product.node.id) isIncluded = true;
                });
        ...
        {isCreated && (
          <div className="text-green-500 pt-2 ">
            <h2 className="font-semibold "> Category Created Successfully!</h2>{" "}
            If it doesn't appear, you can refresh the page to view it
          </div>
        )}
      </form>
    </div>
