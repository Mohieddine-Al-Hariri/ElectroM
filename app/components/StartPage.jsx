...
export const LoadingCard = () => {
  return (
    <div className="relative overflow-hidden rounded-lg h-[200px] max-w-[356px] w-full grow hover:scale-[1.1] duration-200 shadow-lg bg-gray-200 animate-pulse">
...
const StartPage = ({
  ...
  const [lastProductCursor, setLastProductCursor] = useState(
    products ? products[products?.length - 1]?.cursor : ""
  );
  ...
  const addItemsToCart = async (localCart) => {
    ...
    await addManyItemsToCart({ cartId, items, userId: user.id }).then(
      async (res) => {
        const orderItemsPublished = await publishManyItemsAddedToCart(
          res.orderItems
        );
      }
    );
  ...
  useEffect(() => {
    ...
    if (collectionsData) {
      colls = collectionsData.collections.slice(
        0,
        selectedCategory === "Collections & Sales" ? undefined : 4
      );
      colls = colls.map((collection) => {
        const images = collection.node.products?.map(
    ...
    <div>
      <div className="w-full flex justify-center max-sm:items-center items-end  max-sm:gap-2 gap-4 mb-4 max-sm:flex-col fontColor">
        <FilterSelect
          ...
          extraOptions={[
            { name: "Collections & Sales", slug: "Collections & Sales" },
          ]}
          ...
          <div className="flex relative h-40 w-full backGround fontColor text-2xl justify-center items-center rounded-lg ">
            <div className="w-full flex flex-col justify-center items-center fontColor pt-4 ">
              ...
              <div className="flex items-center mt-2 ">
                <a
                  className={` flex justify-center bg-[#25D366] text-white items-center rounded-full aspect-square p-1  `}
                  href="https://wa.me/+96176021231"
                  target="_blank"
                >
                  <svg
                    fill="currentColor"
                ...
                <a
                  href="https://www.instagram.com/electronics_mohie/"
                  target="_blank"
                >
                  <div className="">
                    <Image
